// DECIMAL FIX V4: COMPOUND INTEREST WITH SCALING FIX
// This file contains the complete solution to fix the massive token discrepancy
// while preserving compound interest calculations
...
fn calculate_rewards(
    ...
    // CRITICAL FIX: Apply compound interest but with PROPER DECIMAL SCALING
    // Step 1: Divide rate by 100 to convert from percentage to decimal (e.g., 0.00000125% → 1.25e-8)
    ...
    // Return the properly scaled compound interest result
    raw_rewards
}

// Process harvest instruction with fixed compound interest calculation
fn process_harvest(
    program_id: &Pubkey,
    ...
    // Calculate rewards using compound interest formula with proper scaling
    ...
    // Log reward calculation details for transparency
    msg!("Calculated rewards: {} YOS (raw: {})", 
...
// Process unstake instruction with fixed compound interest calculation
fn process_unstake(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    amount: u64,
    ...
    // Log reward calculation details for transparency
    msg!("Calculated rewards for unstake: {} YOS (raw: {})", 
         raw_rewards as f64 / 1_000_000_000.0,
         raw_rewards);
