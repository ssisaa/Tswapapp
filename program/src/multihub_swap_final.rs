...
// Utility macro for checking data length to avoid panics
macro_rules! assert_data_len {
    ($data:expr, $len:expr) => {
        if $data.len() < $len {
            msg!("Data length too short: expected {}, got {}", $len, $data.len());
...
// Initialize the program with improved error handling and account validation
fn process_initialize(
    ...
    // Check minimum number of accounts
    if accounts.len() < 7 {
        msg!("Error: Not enough accounts for initialization (need at least 7)");
        return Err(MultiHubSwapError::InvalidParameter.into());
    }
    ...
    // Verify addresses
    if !system_program.key.eq(&solana_program::system_program::id()) {
    ...
    if !sysvar::rent::check_id(rent_sysvar.key) {
        msg!("Error: Rent sysvar account is invalid");
        return Err(ProgramError::InvalidAccountData);
    }
    ...
    // Calculate authority PDA and validate with passed bump seed
    let (authority_address, expected_auth_bump) = find_authority_address(program_id);
    msg!("Authority PDA: {}", authority_address);
    
    if authority_bump != expected_auth_bump {
        msg!("Error: Authority bump seed mismatch. Expected: {}, Got: {}", 
            expected_auth_bump, authority_bump);
        return Err(MultiHubSwapError::InvalidAuthority.into());
    }
    ...
    // Calculate required account size for ProgramState
    let space = std::mem::size_of::<ProgramState>();
    msg!("Required program state size: {} bytes", space);
    
    // Get rent from sysvar
    let rent = Rent::from_account_info(rent_sysvar)?;
    let lamports = rent.minimum_balance(space);
    msg!("Rent for program state: {} lamports", lamports);
    
    // Create the program state account if it doesn't exist
    msg!("Creating program state account with seed 'program_state' and bump {}", state_bump);
    
    // Create the program state account using system instruction
    invoke_signed(
    ...
    if program_state_account.data_len() < space {
        msg!("Error: Program state account too small. Required size: {} bytes, actual size: {} bytes",
            space, program_state_account.data_len());
        return Err(MultiHubSwapError::InvalidAccountSize.into());
    }
    
    // Verify the account is owned by our program
    if program_state_account.owner != program_id {
        msg!("Error: Program state account is not owned by our program");
        return Err(MultiHubSwapError::AccountNotOwned.into());
    }
    
    // Verify the account is rent-exempt
    if !rent.is_exempt(program_state_account.lamports(), space) {
        msg!("Error: Program state account is not rent-exempt");
        return Err(MultiHubSwapError::RentNotExempt.into());
    }
    ...
    // Serialize program state to account data with error handling
    match program_state.serialize(&mut &mut program_state_account.data.borrow_mut()[..]) {
        Ok(_) => {
            msg!("Program state serialized successfully");
        },
        Err(err) => {
            msg!("Error serializing program state: {:?}", err);
            return Err(MultiHubSwapError::SerializationFailed.into());
        }
    }
...
// Process swap token instruction (simplified for testing) with improved error handling
fn process_swap_token(
    ...
    // Check minimum number of accounts
    if accounts.len() < 8 {
        msg!("Error: Not enough accounts for swap (need at least 8)");
        return Err(MultiHubSwapError::InvalidParameter.into());
    }
    ...
    msg!("Input token account: {}", user_input_token_account.key);
    msg!("Output token account: {}", user_output_token_account.key); 
    msg!("YOS token account: {}", user_yos_token_account.key);
    msg!("Program state: {}", program_state_account.key);
    ...
    // Verify program state account is owned by our program
    if program_state_account.owner != program_id {
        msg!("Error: Program state account is not owned by our program");
        return Err(MultiHubSwapError::AccountNotOwned.into());
    }
    
    // Get program state with error handling
